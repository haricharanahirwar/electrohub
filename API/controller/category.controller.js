...
export const save=async(req,res)=>{
 ...
 const uploadpath=path.join(__dirname,"../../UI/public/assets/uploads/phones",caticonnm);
...
export const fetch=async(req,res)=>{
   ...
   if(condition_obj!=undefined)
    ...
    condition_obj={};  
   var pList=await CategorySchemaModel.find(condition_obj);
   if(pList.length!=0)
 ...
 export var update=async(req,res)=>{
      ...
      if(obj!=undefined)
      {
         let pDetails = await CategorySchemaModel.findOne(JSON.parse(req.body.condition_obj));
         if(pDetails){
             let project=await CategorySchemaModel.updateOne(JSON.parse(req.body.condition_obj),{$set: JSON.parse(req.body.content_obj)});   
             if(project)
